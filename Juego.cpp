...
void Juego::interactuar(Cazador& cazador) {
	int x = cazador.getPosicionX();
	int y = cazador.getPosicionY();
	if (isla.getMatriz()[x][y]=='X') {
		if (&cazador==&Cazador1) {
			cout << "Cazador 1 (" << Cazador1.getNombre() << ") ha encontrado el tesoro" << endl;
		}
		else if (&cazador == &Cazador2) {
			cout << "Cazador 2 (" << Cazador2.getNombre() << ") ha encontrado el tesoro" << endl;
		}
	} else if (isla.getMatriz()[x][y]=='#') {
		activarTrampa(cazador);
